...
const Chat = ({roomId}) => {
    ...
    return (
        <div className="flex flex-col bg-primary w-[330px] h-full p-6 rounded-3xl justify-between">
            <div className="overflow-y-scroll rounded-3xl" ref={chatRef} onScroll={handleScroll}>
            ...
            <div className="mt-5 justify-center">
                <input className="pl-3 h-[37px] w-full rounded-full placeholder:text-accent placeholder:text-xs" value={message} type="text" placeholder="Type a message" onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => {{
                    if (event.key === "Enter") {
                        if (message !== "") {
                            send_message({body: message, playerId: snap.playerId, roomId: roomId}).catch(error)
                            setMessage("")
                        }
                    }
                }}}/>
