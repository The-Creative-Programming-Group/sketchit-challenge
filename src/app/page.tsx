    ...
    <main className="w-screen flex min-h-screen flex-col items-center justify-between">
      ...
      <form className="flex flex-col gap-6 w-full sm:w-auto max-w-[20rem] sm:max-w-none">
        ...
        <div className="mt-10 flex justify-between">
          <Switch description="Chat" onCheckedChange={checked => {setting.setChat(checked)}}/>
          <SmallInput
            type="number"
            onChange={(e) => {setting.setTimeLimit(parseInt(e.target.value))}}
            value={setting.timeLimit}
            defaultValue={setting.timeLimit}
            description="Time limit"
          />
          <Switch description="Pairplay" onCheckedChange={checked => {setting.setPairplay(checked)}} />
        </div>
      </form>
      <div />
    </main>
